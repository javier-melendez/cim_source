...
function launchProjectile() {
  if (!state.activeProjectile || !state.sling) return;
  if (!canLaunch()) return;
...
function loadNextProjectile() {
  window.clearTimeout(state.flightTimer);
  if (state.activeProjectile) Composite.remove(engine.world, state.activeProjectile);
  state.activeProjectile = null;
  state.sling = null;
  if (state.mode === "finished" || state.turnMode !== "attack") return;
  ...
  if (state.projectileCounts[state.selectedProjectile] <= 0) state.selectedProjectile = available;
...
function handleKeyDown(event) {
  ...
  if (state.mode !== "playing" || state.turnMode !== "attack") return;
  if (!state.activeProjectile || state.activeProjectile.game.launched) return;
  event.preventDefault();
  if (key === " ") {
  ...
  if (key === "arrowup") changeAimAngle(-step);
  if (key === "arrowdown") changeAimAngle(step);
}

function handleKeyUp(event) {
  if (event.defaultPrevented || event.metaKey || event.ctrlKey || event.altKey) return;
  if (event.key !== " ") return;
  if (state.chargeSource !== "keyboard") return;
  event.preventDefault();
  releaseKeyboardLaunchCharge();
}

function positionProjectileAtLauncher() {
  if (state.interaction !== "attack" || !state.activeProjectile || state.activeProjectile.game.launched) return;
...
function keepProjectilesAboveFloor() {
  Composite.allBodies(engine.world).forEach((body) => {
    if (body.game?.kind !== "projectile") return;
...
function handleCollisions(event) {
  event.pairs.forEach((pair) => {
    ...
    if (projectile && block) damageBlockFromProjectile(projectile, block, pair);
...
function damageFallingBlock(block) {
  ...
  if (now < state.fallDamageEnabledAt) return;
  if ((state.lastFallDamage.get(block.id) || 0) + FALL_DAMAGE_COOLDOWN_MS > now) return;
  ...
  if (dealt > 0) floatingText(block.position, `-${Math.round(dealt)}`, "damage", "colateral");
}

function damageBlockFromProjectile(projectile, block, pair) {
  const key = [projectile.id, block.id].sort().join(":");
  const now = performance.now();
  if ((state.lastCollision.get(key) || 0) + CONFIG.collisionCooldownMs > now) return;
  state.lastCollision.set(key, now);
  if (projectile.game.hitBlockIds.includes(block.game.id)) return;
  ...
  if (cfg.explosionRadius && !projectile.game.exploded) explode(projectile, cfg);
  if (block.game.state === "destroyed") spawnFragments(block.position, block.render.fillStyle);
  updateUi();
}
...
function explode(projectile, cfg) {
  ...
  state.blocks.forEach((block) => {
    ...
    if (distance > cfg.explosionRadius) return;
    const falloff = 1 - distance / cfg.explosionRadius;
    const damage = cfg.explosionDamage * falloff * (cfg.multipliers[block.game.level] ?? 1);
    const dealt = applyDamage(block, damage);
    recordBlockDamage(block, dealt);
    if (dealt > 1) floatingText(block.position, `-${Math.round(dealt)}`, "damage", damageEffectivenessText(dealt, block.game.maxHp));
  });
...
function handleRepairClick(event) {
  if (state.mode !== "playing" || state.turnMode !== "defense" || state.interaction !== "repair" || !state.selectedRepair) return;
...
function renderToolbars() {
  els.projectileButtons.innerHTML = Object.entries(CONFIG.projectiles).map(([key, cfg]) => (
    `<button class="tool-button ${state.selectedProjectile === key ? "selected" : ""}" data-projectile="${key}" ${state.projectileCounts[key] <= 0 || state.turnMode !== "attack" || state.attacksUsed >= ATTACKS_PER_TURN ? "disabled" : ""}>${cfg.label}<span>${state.projectileCounts[key]} restantes</span></button>`
  )).join("");
  els.repairButtons.innerHTML = Object.entries(CONFIG.repairs).map(([key, cfg]) => (
    `<button class="tool-button ${state.selectedRepair === key ? "selected" : ""}" data-repair="${key}" ${state.repairCounts[key] <= 0 || state.turnMode !== "defense" || state.defenseMovesUsed >= DEFENSE_MOVES_PER_TURN ? "disabled" : ""}>${cfg.label}<span>${state.repairCounts[key]} restantes</span></button>`
  )).join("");
...
function selectProjectile(key) {
  if (state.turnMode !== "attack" || state.attacksUsed >= ATTACKS_PER_TURN) return;
  if (state.projectileCounts[key] <= 0) return;
  state.selectedProjectile = key;
  message(projectileSummary(key));
  if (state.activeProjectile && !state.activeProjectile.game.launched) loadNextProjectile();
  renderToolbars();
  updateUi();
}

function selectRepair(key) {
  if (state.turnMode !== "defense" || state.defenseMovesUsed >= DEFENSE_MOVES_PER_TURN) return;
  if (state.repairCounts[key] <= 0) return;
...
function unloadLauncher() {
  if (state.sling) Composite.remove(engine.world, state.sling);
  if (state.activeProjectile && !state.activeProjectile.game.launched) Composite.remove(engine.world, state.activeProjectile);
  state.sling = null;
  state.activeProjectile = null;
}
...
function hpBarColor(ratio) {
  if (ratio >= 0.9) return "#2fbf68";
  if (ratio >= 0.72) return "#73c950";
  if (ratio >= 0.55) return "#b9c943";
  if (ratio >= 0.38) return "#d59d2f";
  if (ratio >= 0.2) return "#d8612f";
  return "#b93030";
}

function armorBarColor(ratio) {
  if (ratio >= 0.75) return "#1f7fe5";
  if (ratio >= 0.35) return "#35a7ee";
  return "#73d4ff";
}
...
function recordDamage(amount) {
  if (amount > 0) state.damageReceived += amount;
}

function recordRecovery(amount) {
  if (amount > 0) state.damageRecovered += amount;
}

function damageEffectivenessText(amount, maxHp) {
  const ratio = maxHp ? amount / maxHp : 0;
  if (ratio >= 0.35) return "Es muy eficaz";
  if (ratio >= 0.22) return "Hace bastante daño";
  if (ratio > 0.12) return "Hace daño";
...
function repairEffectivenessText(amount, maxHp) {
  const ratio = maxHp ? amount / maxHp : 0;
  if (ratio >= 0.35) return "Es muy eficaz";
  if (ratio >= 0.22) return "Muy bien jugado";
  if (ratio > 0.12) return "Bien jugado";
...
function releaseLaunchCharge(event) {
  if (!state.charging || state.chargeSource !== "pointer") return;
  event.preventDefault();
...
function releaseKeyboardLaunchCharge() {
  if (!state.charging || state.chargeSource !== "keyboard") return;
  releaseChargedProjectile();
}
...
function stopLaunchCharge() {
  ...
  if (state.chargeFrame) window.cancelAnimationFrame(state.chargeFrame);
...
function firstAvailableValidRepair() {
  const integrity = calculateIntegrity(state.blocks);
  return Object.keys(CONFIG.repairs).find((key) => (
    state.repairCounts[key] > 0 && state.blocks.some((block) => canRepair(block, key, integrity).ok)
  )) || null;
}
...
function countCollapsedLevels(integrity) {
  return Object.values(integrity).filter((value: any) => value <= 0).length;
}
...
function setMeter(key, value) {
  (document.querySelector(`#meter-${key}`) as HTMLMeterElement).value = value;
...
function reflection(integrity, weakest) {
  if (integrity.identity <= 20) return "Sin una identidad sólida, la imagen y la reputación pierden su soporte.";
  if (integrity.image <= 25) return "La percepción pública debilitada dejó expuesta la reputación.";
  if (integrity.reputation >= 70) return "Una estructura coherente permitió resistir la crisis.";
  if (integrity.reputation <= 25 && integrity.identity >= 65) return "La reputación puede dañarse rápidamente, pero una identidad sólida facilita su recuperación.";
