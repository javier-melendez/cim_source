...
export function canRepair(block, materialKey, integrity) {
  ...
  if (!block?.game || block.game.kind !== "castleBlock") return { ok: false, reason: "Selecciona un bloque del castillo." };
  if (block.game.state === "destroyed") return { ok: false, reason: "Ese bloque ya no se puede reparar." };
  if (block.game.hp >= armorLimit(block)) return { ok: false, reason: "El bloque ya está en su armadura máxima." };
  return { ok: true };
}

export function repairAmount(block, materialKey, integrity) {
  const material = CONFIG.repairs[materialKey];
  if (!material.allowed.includes(block.game.level)) return Math.min(1, armorLimit(block) - block.game.hp);
